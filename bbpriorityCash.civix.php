 ...
 * extension.
 */
class CRM_BbpriorityCash_ExtensionUtil {
  const SHORT_NAME = 'bbpriorityCash';
  const LONG_NAME = 'info.kabbalah.payment.bbpriorityCash';
  const CLASS_PREFIX = 'CRM_BbpriorityCash';
...
use CRM_BbpriorityCash_ExtensionUtil as E;

($GLOBALS['_PathLoad'][0] ?? require __DIR__ . '/mixin/lib/pathload-0.php');
pathload()->addSearchDir(__DIR__ . '/mixin/lib');
spl_autoload_register('_bbpriorityCash_civix_class_loader', TRUE, TRUE);

function _bbpriorityCash_civix_class_loader($class) {
  if ($class === 'CRM_BbpriorityCash_DAO_Base') {
    if (version_compare(CRM_Utils_System::version(), '5.74.beta', '>=')) {
      class_alias('CRM_Core_DAO_Base', 'CRM_BbpriorityCash_DAO_Base');
      // ^^ Materialize concrete names -- encourage IDE's to pick up on this association.
    }
    else {
      $realClass = 'CiviMix\\Schema\\BbpriorityCash\\DAO';
  ...
  if (strpos($class, 'CiviMix\\Schema\\BbpriorityCash\\') === 0) {
...
function _bbpriorityCash_civix_mixin_polyfill() {
...
function _bbpriorityCash_civix_civicrm_config($config = NULL) {
  ...
  _bbpriorityCash_civix_mixin_polyfill();
}
 ...
 */
function _bbpriorityCash_civix_civicrm_enable(): void {
  _bbpriorityCash_civix_civicrm_config();
  _bbpriorityCash_civix_mixin_polyfill();
}
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _bbpriorityCash_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
